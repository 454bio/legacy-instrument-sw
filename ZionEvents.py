...
def initializeSession():
	...
	myEventList = create_event_list(Blue_Timing, Orange_Timing, UV_Timing, Camera_Capture_Times, Repeat_N)
	# Creates GPIO controller:
	myGPIO = ZionGPIO()
	# Creates camera:
	myCamera = ZionCamera(Spatial_Res, Frame_Rate, ISO, Exposure_Mode, Shutter_Speed, Auto_White_Balance, Stabilize, gpio_ctrl=myGPIO)
	return (myGPIO, myCamera, myEventList)
...
def create_event_list(LED_Blu_Timing, LED_Or_Timing, LED_UV_Timing, Camera_Capture_Times, Repeat_N=0):
	...
	eventList += [(capture_event[0], 'take_snapshot', capture_event[1]) for capture_event in Camera_Capture_Times]	
...
def performEvent(event, camera, gpio_ctrl, capture_file_name):
	event_type=event[1]
	if event_type == 'take_snapshot':
		if event[2]:
			camera.capture(capture_file_name, cropping=event[2], useIndex=True)
		else:
			camera.capture(capture_file_name, useIndex=True)			
...
def performEventList(eventList, camera, gpio_ctrl, Repeat_N=0, baseFilename='capture_file_'):
	...
	for n in range(Repeat_N+1):
		...
		for e in range(len(eventList)-1):
			event = eventList[e]
			performEvent(event, camera, gpio_ctrl, baseFilename)
			sleep((eventList[e+1][0]-event[0])/1000.)
		#after for loop, now perform last event:
		performEvent(eventList[-1], camera, gpio_ctrl, baseFilename)
