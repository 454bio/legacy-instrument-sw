import time
from dataclasses import dataclass, fields, asdict, is_dataclass, make_dataclass
...
_ZionProtocolLEDFieldNames = tuple(f"led_{color.name}" for color in ZionLEDColor)

@dataclass(slots=True)
class _ZionProtocolEntry():
    is_event: bool
    name: str
    cycles: int
    capture: bool
...
# Create directly accessible led entries
ZionProtocolEntry = make_dataclass(
    '_ZionProtocolLEDFields',
    [(field, int) for field in _ZionProtocolLEDFieldNames],
    bases=(_ZionProtocolEntry,),
    slots=True,
)

# _ZionProtocolEntrySlots = tuple(map(attrgetter('name'), fields(_ZionProtocolEntry))) + _ZionProtocolLEDFieldNames

# @dataclass(slots=True)
# class ZionProtocolEntry(_ZionProtocolLEDFields, _ZionProtocolEntry):
#     pass
...
# class ZionGtkTreeStore(Gtk.TreeStore):
...
#         self._treestore = ZionGtkTreeStore(*self._treestore_helper.types)
